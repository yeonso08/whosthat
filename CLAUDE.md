...
**남은 것은 둘이다: 백업**(아래 "데이터는 어디에 있나")**과 애드센스 재신청 준비**(기수 소개는 배포 끝 → 직업·나이 채우기 → 재신청). 그 밖에 계속 하는 일은 계정 채우기(솔로지옥 2자리·나는 솔로 87자리)와 사진 올리기이고, 둘 다 이제 관리자 화면에서 한다 — 코드 작업이 아니다. 언어를 더할 때 절차는 위 "언어를 하나 더할 때", 프로그램을 더할 때는 `src/data/README.md` 의 "프로그램을 추가할 때".
