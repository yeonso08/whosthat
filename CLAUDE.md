...
- **포스터의 그림은 이름 그 자체다. 프로그램 포스터는 앞으로도 못 싣는다** — 방송사·제작사가 저작권자라 방송 캡처를 안 쓰는 것과 같은 이유로 막혀 있다(`PLANNING.md` §9 ①). 그러니 이 자리를 "사진 오면 채울 빈 판" 으로 두지 말 것. 활자로 완성해야 하는 판이고, 한국 예능 포스터가 원래 활자로 그려진다는 점에서도 방향이 같다.
...
- **비율은 3:4 다. 넷플릭스·티빙의 2:3 을 그대로 쓰지 않는다** — 그쪽 판은 그림이 꽉 차 있고 이쪽은 활자만 있어서, 같은 비율로 두면 이름 위가 통째로 빈 자국이 된다.
