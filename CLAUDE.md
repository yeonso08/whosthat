...
- **프로그램마다 다른 말은 사전의 `site.programs` 에 문장째로 있다.** 이름·기수 라벨·목록 제목(`전체 기수` / `전체 시즌`)·`지난 기수`·`최신 기수`·요약 두 줄·검색 안내 두 줄이다. **낱말만 표로 두고 문장에 끼워 넣지 말 것** — 어순이 다른 언어에서 반드시 어색해진다(가명·특집 표와 같은 이유). 꺼내는 곳은 `programStrings(programId, locale)` 하나다.
