...
- **`Season.label` 은 없다. `Season.number` 뿐이다.** 화면에 뜨는 말은 프로그램마다 다르다 — `33기` 와 `시즌 4` 다. 라벨을 데이터에 저장하면 언어 × 프로그램만큼 적어야 하므로, 번호만 두고 DB 의 프로그램 문구(`seasonLabel`)가 만든다(`localizeSeasonLabel`). 정렬도 이 번호를 본다.
- **프로그램마다 다른 말은 DB 에 문장째로 있다**(관리자에서 편집한다). 이름·기수 라벨·목록 제목(`전체 기수` / `전체 시즌`)·`지난 기수`·`최신 기수`·요약 두 줄·검색 안내 두 줄이다. **낱말만 표로 두고 문장에 끼워 넣지 말 것** — 어순이 다른 언어에서 반드시 어색해진다(가명·특집 표와 같은 이유). 꺼내는 곳은 `programStrings(programId, locale)` 하나다.

- **사전에 문구가 없는 프로그램은 `getPrograms()` 가 화면에서 걸러낸다**(`hasProgramStrings`). 관리자 화면에서 프로그램을 만들면 DB 에는 바로 생기는데 사전은 코드라 배포 전까지 비어 있다 — 예전엔 그 상태가 화면까지 흘러가 `programStrings` 가 던졌고, **이미 떠 있는 홈이 500 이 됐다**(2026-09-07 에 재현해서 확인했다). 번역 하나가 없다고 사이트를 내리는 건 어느 CMS 도 하지 않는 동작이다(Payload 는 `fallback: true` 가 기본값이다). 지금은 조용히 빠지고 빌드 로그에 경고만 남는다 — **오타로 진짜 프로그램이 사라지는 경우**를 그 경고가 잡는다.
- `programStrings` 자체는 아직 던진다. 걸러내는 장치가 깨졌을 때 조용히 지나가지 않게 남겨 둔 것이라 **없애지 말 것.**
- **세 언어 중 하나만 확인해도 된다** — `Dictionary = typeof ko` 라서 사전 세 벌이 같은 프로그램 키를 갖도록 타입이 강제한다(하나만 빼면 컴파일 에러가 난다).
- **`alias` 는 "방송에서 불린 이름" 이다.** 나는 솔로는 가명(`영수`), 솔로지옥은 실명(`최시훈`) 이 들어간다. 필드를 나누지 않은 이유는 화면이 하는 일이 같기 때문이다 — 크게 부르는 이름 하나와, 그 아래 실명·나이·직업 줄이다. 로마자·가타카나 표기는 **DB 의 `alias_translations` 한 표**에 프로그램 구분 없이 쌓인다(한글 원문이 키다 — "영수" 를 33명이 함께 쓰므로 출연자 행에 컬럼으로 넣지 않는다). 관리자에서 새 이름을 넣으면 그 자리에서 함께 묻는다. 실명이 들어가는 프로그램은 `name`(실명) 을 비워 둔다 — 같은 값이 두 줄로 나온다.
...
- **데이터에서 나온 말은 사전이 아니라 DB 다**(2026-09-07 에 옮겼다): 이름 번역 87개(`alias_translations`), 특집 13개(`special_translations`), 프로그램 문구(`program_strings`). 관리자에서 편집하고, **없으면 한국어 원문으로 떨어진다** — 화면이 깨지는 것보다 번역이 안 된 채 뜨는 게 낫다. 가르는 기준은 "프로그램을 하나 더 추가할 때 이 문구도 새로 써야 하나" 다: 그렇다면 DB, 아니면 사전.
...
네 곳이다: `locales.ts` 의 `LOCALES`·`LOCALE_NAMES`, `src/dictionaries/<code>.json` 한 벌, `i18n.ts` 의 `OG_LOCALES`, 그리고 **서체 두 자리**. (프로그램 문구와 이름·특집 번역은 DB 라 관리자 화면에서 그 언어 탭을 채운다 — 코드가 아니다.) 화면·sitemap·hreflang·정적 생성은 그 목록을 따라가므로 따로 손댈 게 없다 — `Record<Locale, …>` 로 못박은 표들이 빠진 칸마다 컴파일 에러를 낸다.
...
- **여백·포커스 링·눌림처럼 여러 화면이 공유하는 값은 `globals.css` 의 `@utility` 다** — `gutter`(화면 좌우 여백, 위 "화면 폭" 절), `focus-ring`, 그리고 `lift`·`press`(아래 "디자인" 절)가 그것이다.
...
- **여백은 `gutter` 하나가 정한다**(`globals.css`). 열 곳에 `px-5` 로 흩어져 있던 것을 모았다 — 한 곳에서 정해야 헤더·격자·목록·푸터가 같은 세로선에 선다. (짝이던 `gutter-inset` 은 없앴다 — 줄 목록이 `ListCard` 안으로 들어가면서 줄이 판 끝까지 차게 됐고, 판의 여백은 `ListCard` 가 `gutter` 로 건다.)
- **읽는 화면은 좁은 기둥, 격자 화면은 컨테이너 전체다.** 프로그램 화면(`max-w-[860px]`)과 정책 두 페이지(`max-w-[68ch]`)는 가운데 기둥으로 서고, 홈의 포스터 진열대와 기수 상세의 출연진 격자만 1280px 을 다 쓴다. 1280px 짜리 목록 줄은 왼쪽 끝 제목과 오른쪽 끝 숫자로 찢어져서 한 줄인데 두 덩어리로 읽히고, 1280px 짜리 산문 한 줄은 눈이 다음 줄 첫 글자를 못 찾는다.
- **홈만 가운데 정렬이다**(2026-08-31). 프로그램이 둘뿐이라 왼쪽 정렬로는 오른쪽 절반이 통째로 비는데, 그건 열을 늘리거나 판을 키워서 메울 수 있는 종류가 아니다. 안쪽 화면은 그대로 왼쪽 정렬이다 — 읽고 훑는 화면에는 눈이 돌아갈 세로선이 있어야 한다.
- **홈의 포스터는 격자가 아니라 가운데로 모이는 진열대다**(`flex-wrap justify-center` + `POSTER_WIDTH` 고정 폭). 컨테이너를 열로 나누면 둘뿐인 지금 판이 손톱만 해지고, 스무 개가 되면 줄이 알아서 접히면서 마지막 줄도 가운데에 선다. **기수 상세의 출연진 격자만 2 → 3 → 4 → 5 열이다.**
- **`next/image` 의 `sizes` 가 판 크기와 짝이다** — 안 맞추면 화면은 멀쩡한데 필요 이상으로 큰 파일이 내려간다. 눈으로는 안 보이는 종류라 폭이나 열 수를 고칠 때 `POSTER_WIDTH`·`POSTER_SIZES`·`CARD_SIZES` 를 같이 연다.
...
- **포스터 판의 `방영 중` 은 글자가 아니라 점만 찍는다**(`sr-only` 로 읽어 준다). 판이 좁아 글자가 들어갈 자리가 없다. **다만 홈이 "어느 기수가 방영 중인지" 를 말하지 않던 규칙은 뒤집혔다**(2026-08-31) — 히어로의 방영 중 줄(`LiveRow`)이 그걸 말한다. 그 규칙은 좁은 판의 사정에서 나온 것이었지 홈이 알아선 안 되는 정보라서가 아니었다.
...
- **겹친 원의 테두리는 두 곳 다 `border-card` 다**(2026-08-31 에 `border-background` 에서 바꿨다). 기수 줄이 `ListCard` 안으로 들어가면서 원이 놓이는 자리가 히어로·목록 줄 둘 다 흰 판 위가 됐다 — 배경색 테두리는 배지 바탕(`bg-elevated`)과 명도가 거의 같아 겹친 원을 갈라 주지 못한다. 판 색이라야 겹친 자리에 흰 선이 지나가 원이 하나씩 세어진다. **짝인 규칙이 하나 더 있다**: `CastAvatar` 의 세 상태가 전부 `bg-elevated` 라야 한다(둘이 `bg-card` 이던 시절엔 흰 판 위에서 원이 통째로 사라졌다). 상태를 가르는 건 바탕이 아니라 글자의 농도다.
...
- 사이트는 **라이트·다크 두 팔레트를 다 가진다**(2026-08-20 부로 "다크 전용" 원칙 폐기). `globals.css` 의 `:root` 가 라이트, `.dark` 가 다크다 — 둘 다 위 시안 D 팔레트를 흑백 축으로 그대로 짝지은 것이라 명도만 뒤집혔지 구조는 같다. `next-themes` 로 전환하고(`ThemeProvider`, `attribute="class"`), 기본값은 여전히 `dark`다 — 원래 시안의 첫인상을 지키려는 것이다. 전환 버튼(`ModeToggle`)은 상단 바(`SiteNav`)에 있다 — 워드마크·언어 선택과 한 줄이고, 레이아웃에 있어 페이지마다 그 줄을 반복하지 않는다.
- **유채색은 둘뿐이고, 둘 다 자리가 못박혀 있다**: `--verified`(앰버)는 확인된 계정의 핸들, `--live`(빨강)는 방영 중 점. 강조·CTA·배지 같은 다른 용도로 번지게 하지 말 것 — **셋째 자리가 생기는 순간 둘 다 신호이길 그만둔다.**
...
- **`방영 중` 은 빨간 점이다**(`LiveDot`, 2026-08-31). 그전에는 명도만으로 세웠는데(`bg-foreground` 점) 방영 중인지 아닌지가 화면에서 거의 안 읽혔다. 빨강은 방송의 라이브 표시라 설명이 필요 없는 기호고, `--verified` 를 빌려 쓰지 않으므로 "확인" 과 "방영" 두 축도 안 뭉갠다.
- **빨강은 점에만 쓰고 글자에는 안 쓴다.** 글자까지 물들이면 붉은 면적이 앰버 핸들과 경쟁하는데, 핸들은 방문자가 가지러 온 값이라 그 싸움에서 지면 안 된다. "방영 중" 이라는 사실은 그 옆 글자가 그대로 말한다.
- **점에는 천천히 퍼지는 무리가 붙는다**(`live-halo`, 2.4s). 정지한 빨간 점은 상태 배지지만, 무리가 있으면 "지금" 이 된다 — 이 사이트에 사람이 오는 순간이 방송 직후라 그 구분이 실제로 값이 있다. 모션을 줄인 사람에게는 무리가 안 움직이고 옅게 멈춰 선다.
- **중립색은 차가운 쪽이다**(다크 `#0a0a0c` 순흑, 라이트 `#fafafa`). 한동안 따뜻한 쪽(`#161511`)으로 굴려 뒀었다 — 사진이 0장인 동안 순흑 화면이 통째로 평평해 보인다는 이유였다. **그 문제는 색이 아니라 깊이의 문제였고, 지금은 그림자와 카드 명도차가 판을 띄워서 해결한다**(2026-08-31 에 되돌렸다). 따뜻한 바탕은 아카이브라는 성격과도 어긋났다 — 이 화면의 주인공은 사진과 핸들이지 종이 질감이 아니다.
- **다만 라이트의 `--card` 는 `#ffffff` 다**(팔레트가 원래 쓰던 `#f1f1f3` 이 아니다). 그 값은 배경(`#fafafa`)보다 **어두워서** 카드가 파인 면으로 읽힌다 — 그림자로 판을 띄우는 지금 구성과 정반대다. 카드는 바탕보다 밝아야 뜬다.
...
### 면·활자·움직임 (2026-08-31 재디자인)

화면이 "딱딱하고 투박하다" 는 지적을 받고 통째로 다시 짰다. 시안 D 의 방향(사진이 화면을 채우는 브라우징 문법, 흑백 + 유채색 하나)은 그대로고, 그 위에 얹히는 면·활자·움직임 세 겹만 바꿨다. **원인은 취향이 아니라 아래 셋이었다.**

- **면은 실선이 아니라 그림자로 세운다**(`--soft-shadow`·`--lifted-shadow`, `shadow-soft`·`shadow-lifted`). 1px 테두리로만 그린 판은 배경에 오려 붙인 종이로 보인다. 그림자는 두 겹이다 — 짧고 진한 것(판의 두께)과 길고 옅은 것(바닥의 그늘)을 겹쳐야 얼룩이 아니라 떠 있는 면이 된다. **다크는 값이 따로다**(훨씬 진하게) — 라이트 값을 그대로 쓰면 어두운 판에서는 그림자가 아예 안 보인다.
- **제목 무게를 900 → 700 으로 내리고 크기를 키웠다**(`tracking-tighter` → `-0.03em`). 한글에서 그 조합은 획이 붙어 제목이 도장처럼 찍힌다. **위계는 굵기가 아니라 크기 차이와 여백이 진다.** 작은 글자(11px·11.5px)도 12px 로 올렸다.
- **눌리는 것은 `lift`·`press` 두 유틸리티로만 반응한다.** 판(포스터·기수 카드·출연진 카드)은 뜨고(`lift`), 줄과 버튼은 살짝 눌린다(`press`). 그전에는 `hover:opacity-85` 하나였는데, 투명도만 깎이는 건 "판이 흐려졌다" 지 "판을 집었다" 가 아니다. **`prefers-reduced-motion` 에서는 움직임만 빼고 그림자 변화는 남긴다** — 어느 판을 가리키는지는 그대로 보여야 한다.
- **홈 히어로는 "검색창 + 지금 방영 중" 이다**(`LiveRow`). 이 사이트에 사람이 오는 순간은 방송 직후 "저 사람 누구야" 이고, 그때 찾는 기수는 거의 언제나 지금 나가는 기수다 — 그전 홈은 프로그램을 고르고 → 기수를 고르고 → 사람을 찾는 세 단계였다. 방영 중인 줄은 그 셋을 한 번으로 줄인다. **현황(`0 / 12 확인`)을 숨기지 않는다** — 방영 중인 기수는 계정이 잠겨 있어 대개 비어 있고, 눌러서 알게 하는 것보다 누르기 전에 말하는 편이 이 사이트가 파는 신뢰와 맞는다. 방영 중인 게 없으면 줄이 통째로 사라진다(빈 자리를 "없음" 으로 채우면 히어로가 안내문이 된다). 검색어를 치면 결과에 자리를 내준다 — 검색창의 `children` 이라 저절로 그렇게 된다.
- **그 줄의 바탕은 `bg-elevated` 다.** 바로 위 검색창이 `bg-card`(흰 판)라, 같은 색으로 두면 흰 판 아래 붙은 흰 판이 되어 검색 자동완성 목록으로 읽힌다. 이건 입력의 결과가 아니라 따로 선 칩이다.
- **줄 목록은 판(`ListCard`) 안에 담는다.** 배경 위에 그냥 뜬 줄들은 왼쪽 제목과 오른쪽 숫자 사이가 비어 목록으로 안 읽혔다. 판이 생기면 구분선이 짧아지고 목록 전체가 한 덩어리로 잡힌다. 기수 목록과 검색 결과 두 묶음이 같은 판을 쓴다.
- **출연진 카드는 정사각형이고, 아래 그러데이션은 사진이 있을 때만 깐다.** 세로로 길었던 때는 사진이 0장인 동안 격자가 큰 빈 상자 열댓 개였다. 정사각형은 인스타 프로필 사진이 안 잘리는 비율이라 사진이 들어와도 그대로 쓴다. 그러데이션은 사진 위에서 글자를 읽히게 하는 장치라, 없는 판에 깔면 띠가 세 겹 생겨 판만 탁해진다 — **글자 블록의 자리와 모양은 어느 쪽이든 같다**(위 규칙 그대로다).
- **최신 기수 카드는 위·아래 두 단이다** — 위는 무엇인지(상태 줄 + 이름), 아래는 누가 나왔고 얼마나 확인됐는지다. 현황에는 막대를 하나 붙인다(`12 / 14` 는 읽어야 알지만 막대는 안 읽어도 보인다). **막대에 색을 쓰지 않는다** — 명도만으로 찬 만큼과 빈 만큼이 갈린다. 명단이 없는 기수(`total === 0`)는 막대를 아예 안 그린다.
- **상단 바에는 프로그램 링크를 두지 않는다**(`SiteNav` 는 워드마크·언어·라이트다크 셋뿐이다). 둘일 때는 한 줄에 들어갔지만 프로그램은 계속 는다 — 이름을 하나씩 붙이는 자리는 어느 개수에서 반드시 넘치고, 거기에 "더보기" 를 달면 상단 바가 두 번째 목록 화면이 된다. 프로그램 목록은 홈이 지고, 안쪽 화면은 제목 옆 되돌아가기로 한 단계씩 올라간다.
...
**프로그램은 둘이다**(2026-08-31). 나는 솔로는 1~33기 골격과 명단이 다 들어가 있고(408명, 2026-08-21) 계정은 320명이 `found`, 나머지가 `searching` 이다 — 아직 못 채운 건 방영 중이라 계정이 잠긴 33기와 각 기수에 한둘씩 남은 자리다. **솔로지옥은 2026-09-02 에 시즌 1~5 명단 66명을 채웠다**(직업까지, 계정은 전부 `searching`) — 색인에서 빠져 있던 상태가 그때 풀렸다. 명단은 나무위키와 한국 기사로 교차 확인했다. **영어 위키백과의 시즌 2·3 여성 명단은 틀렸다**(`서서은`→실제 `최서은`, `이민수`→`임민수`, `이시은`→`유시은`, `김민지`→`조민지`) — 영어 자료만 보고 채우지 말 것. 2026-09-02 에 시즌 5 부터 시즌 1 까지 계정을 돌려 **66명 중 64명을 확보했다** — 대부분 종영 인사 게시물이다("안녕하세요 ○○입니다… 솔로지옥5"). **실명 방송이라 나는 솔로보다 확보가 쉽다**: 가명과 달리 이름으로 바로 검색이 되고, 넷플릭스 출연자는 종영 직후 인사 글을 거의 다 올린다. 김재진·조이건은 본인이 밝힌 근거가 없어 `searching` 으로 남겼다 — 계정 후보는 있지만(조이건은 팔로워 64.9만 실명 계정) 프로필·게시물 어디에도 솔로지옥을 안 걸어 뒀다. 솔로지옥 쪽은 이 둘만 남았고, 다음 일은 나는 솔로에 남은 87자리(방영 중인 33기 12명 포함)다.
...
**광고(구글 애드센스)를 붙였다**(2026-08-24). 값은 두 곳이 짝이다 — `lib/site.ts` 의 `ADSENSE_CLIENT_ID`(`ca-pub-…`)와 `public/ads.txt`(`pub-…`). 번호가 어긋나면 애드센스가 "승인되지 않은 판매자"로 잡아 수익이 막힌다. 스크립트는 아래 절대로 **페이지가** 걸고, ID 가 비면 아예 안 건다 — 틀린 ID 로 요청이 나가는 게 안 나가는 것보다 나쁘다.

**광고는 콘텐츠가 있는 화면만 건다**(`components/ads.tsx` 의 `AutoAds`). 2026-09-02 에 애드센스가 "게시자 콘텐츠가 없는 화면에 Google 게재 광고" + "가치가 별로 없는 콘텐츠" 로 정책 위반을 걸었다 — 스니펫이 루트 레이아웃에 있어서 명단이 빈 기수·골격만 있는 프로그램·정책 두 페이지에도 광고가 나가고 있었다. 색인은 `isIndexable` 로 빼 두고 광고만 그대로 나간 것이다. 지금은 **페이지가 직접 `<AutoAds />` 를 건다**: 기수 상세는 `hasAdContent(season)`(확인한 계정 1건 이상), 프로그램 화면은 `programHasAdContent(program)` 이고, **홈은 조건 없이 걸고, 소개·FAQ·정책 두 페이지에는 안 건다.** 소개·FAQ 는 처음에 무조건 거는 쪽으로 넣었다가 같은 날 뗐다 — 둘 다 명단이 아니라 설명이라 `hasAdContent` 기준이 안 맞고, 무엇보다 "콘텐츠를 광고용으로 부풀리지 않는다"고 적어 둔 페이지에 광고를 다는 게 앞뒤가 안 맞는다. 광고 기준은 색인 기준보다 한 단계 좁다 — 명단만 있고 계정이 0인 기수는 색인에 올릴 만해도 광고를 얹을 화면은 아니다. **새 화면을 붙일 때 이 컴포넌트를 빠뜨리면 광고가 안 나갈 뿐 위반은 안 생긴다** — 빠뜨렸을 때 안전한 쪽이 기본값이다.

**홈은 2026-09-03 에 도로 넣었다.** 거절 직후 조심하느라 뺐는데, 그때 걸린 건 "명단이 비어 화면에 한 문장뿐인" 화면들이었지 홈이 아니다 — 홈에는 사이트가 무엇을 하는지 적은 줄, 방영 중인 기수, 프로그램 판이 있다. 그리고 **애드센스에 등록된 사이트가 apex(`nukko.net`)라 심사 봇이 착지하는 첫 화면이 홈이다** — 거기에만 코드가 없으면 "사이트에서 광고 코드를 찾지 못했다" 쪽으로 잘못 읽힐 자리가 생긴다.
...
**운영 상태**(2026-09-22): **두 번 거절당했고, 3차 재신청을 준비 중이다**(아래 "3차 재신청 준비"). 2026-08-24 에 소유권 확인·검토 요청·GDPR 동의 메시지까지 끝냈고, 2026-09-02 에 "게시자 콘텐츠가 없는 화면에 Google 게재 광고" + "가치가 별로 없는 콘텐츠" 로 거절당했다. 같은 날 위 두 절대로 고쳐(광고를 페이지가 걸게 바꾸고, 소개·FAQ 산문을 세 언어로 넣었다) 재신청했다.

**2차 거절**(2026-09-21): "콘텐츠가 없는 화면에 광고" 는 사라지고 **"가치가 별로 없는 콘텐츠" 하나만 남았다** — 광고를 페이지가 걸게 바꾼 건 통했다. 남은 원인은 기수 상세가 **가명과 핸들뿐인 외부 링크 목록**이라는 것이다(실측: 기수 상세 99단어, 홈 66단어, 소개 817단어). 설명 없이 다른 사이트로 가는 링크만 모은 페이지가 구글 가이드라인의 저품질 전형이고, 목록이 방문자에게 얼마나 쓸모 있는지는 심사가 안 본다. 카카오 애드핏으로 가는 길도 검토했지만 **애드센스를 계속 노리기로 했다.** 기수 화면에 사이트가 직접 쓴 글을 더한다:

- **기수 소개 문단**(`season_descriptions`, 2026-09-21) — 특집 설명·방영 기간·회차·촬영지·출연 구성·최종 선택 결과. **방송에 나온 사실만** 적고 방송 뒤의 연애·결혼·결별은 적지 않는다(일반인의 사생활이고 시간이 지나면 틀린 말이 된다). 38개 기수 × 3언어 초안은 `nukko-admin-api/scripts/season_descriptions.json` 에 출처와 함께 있다. 최종 선택은 기사로 교차 확인했지만 **회차·촬영지는 위키백과 하나가 출처고 그 위키의 날짜가 한 곳 틀렸다**(2기 마지막 회: 기사 10월 6일, 위키 9월 29일) — 그래서 날짜는 월 단위로만 쓴다. 솔로지옥 시즌 1 은 네 번째 커플을 못 확정해 세 쌍만 적었다(`review: true`).
- **나는 솔로 출연자 직업·나이 — 20~32기 채움**(2026-09-22, 165명 중 164명. 23기 정숙은 통편집이라 방송에 나온 적이 없다). **1~19기와 33기는 아직이다** — 초기 기수일수록 자료가 없다. 에이전트 5개가 기수를 나눠 조사했고, 데이터·출처는 `nukko-admin-api/scripts/cast_profiles.json`, 넣는 건 `import_cast_profiles.py`. 규칙(사용자가 정했다):
  - **직업은 방송 자막 그대로, 회사·학교 이름까지.** 나이는 `방영 시작 연도 − 출생 연도`(생일에 따라 만 나이와 1살 차이 날 수 있다).
  - **출처가 엇갈리면 3곳 이상을 보고 많은 쪽**(출생 연도 4건이 이렇게 정해졌다). 한 곳뿐이면 비운다.
  - **방송이 가린 회사 이름은 풀지 않는다**(`S전자`·`L전자`·`H제철`·`P대학교`·`G사`). 블로그들이 가린 이름을 **짐작해서** 채운다 — 23기 영철의 `K자동차` 를 블로그들이 `기아` 와 `KG모빌리티` 로 갈라 적었다. 그래서 회사 이름이 든 항목은 **방송을 받아 쓴 기사**로 다시 확인해 29건을 가린 표기로 되돌렸다. "많은 쪽을 따른다" 는 출처끼리 다른 사실을 말할 때 쓰는 규칙이지, 가린 것과 푼 것 사이에 쓰는 게 아니다. 방송에서 본인이 직접 말한 이름(`현대자동차`·`LG에너지솔루션`·`현대제철`·`삼성전자`·`서울교통공사`)만 그대로다.
  - **32기 상철**은 방송 뒤 "자기소개 과장" 의혹 기사가 났지만(스포츠서울) 사용자가 방송에 나온 대로 두기로 했다.
  - 직업은 번역표가 없어 영어·일본어 화면에도 한국어로 나간다(솔로지옥이 이미 그렇다).
- **38개 소개가 같은 틀에서 나온 문장이다.** 자동 생성 콘텐츠로 읽힐 여지가 있어서 특이한 기수(1·7·13·19·28기)에는 그 기수만의 문장을 넣었다. 기수마다 사람이 쓴 한 줄을 더하는 게 다음으로 효과가 크다.
- **재신청은 소개 문단이 배포되고 색인된 뒤에 한다.** 고치지 않고 다시 넣으면 같은 결과가 나오고, 거절이 쌓이면 다음 심사까지 텀이 길어진다.

**기수 소개 — 배포 끝**(2026-09-22). 코드(nukko-admin-api#1, nukko-admin#1, whosthat#59·#60)와 운영 작업이 다 끝나 **38개 기수 × 3언어(114행)가 실제 사이트에 나온다**. 순서는 이랬다 — 다음에 테이블을 더할 때도 같다:
...
4. 누꼬 반영은 그때 재검증 신호로 했다. 지금은 그 창구가 없어졌고 **관리자 화면의 "사이트에 반영"**(백엔드 `POST /admin/publish` → Deploy Hook)이 같은 일을 한다 — 아래 "데이터는 어디에 있나". 데이터 스크립트(`import_*.py`)는 넣은 뒤 반영 대기 수만 올리므로, 넣고 나서 그 버튼을 누른다.

**아직 남은 것**: 솔로지옥 시즌 1 은 네 커플 중 세 쌍만 이름이 적혀 있다(네 번째를 확인하면 관리자에서 고친다). 다음은 직업·나이(20~32기 끝, 1~19·33기 남음), 마지막이 애드센스 재신청(색인 후).

**3차 재신청 준비**(2026-09-22). 코드·데이터 작업은 여기까지 했다 — 기수 소개 38개(×3언어), 20~32기 직업·나이 164명, 카드에서 나이를 이름 옆 `34세` 로(숫자만 있으면 나이인지 안 읽혔다). 실측 단어 수는 **22기 상세 99 → 197**, 소개만 붙은 5기 122, 홈은 66 그대로다. **바로 재신청하지 않는다** — 심사는 구글이 크롤링해 둔 페이지를 보는데 새 내용이 오늘 올라갔다. 순서는:

1. Search Console 에서 기수 상세 5~10개(20~32기 위주)를 **색인 생성 요청**한다.
2. 1~2주 뒤 같은 곳의 "크롤링된 페이지" 에 소개 문단이 보이면 **재신청**한다.
3. 또 거절되면 **사유가 바뀌었는지**를 먼저 본다. 같으면 1~19기 직업·나이와 기수마다 사람이 쓴 한 줄을 채운다.

통과를 장담할 근거는 없다 — 사이트가 한 달 남짓이고, 방문자가 적고, 이미지가 0장이다(아래 ③).

**재심사를 앞두고 본 위험 셋 중 하나는 고쳤다.** ~~① 홈에 광고가 없다~~ — 2026-09-03 에 도로 걸었다(위 "광고" 절). 남은 하나는 이렇다: ③ **이미지가 0장이다** — 출연자 사진도 프로그램 포스터도 아직 한 장도 없어서 전 화면이 활자뿐이다.

소유권은 **ads.txt 방식**으로 통과했다 — 코드 스니펫 방식은 실패했는데, 애드센스에 등록된 사이트가 apex(`nukko.net`)고 실제 사이트는 `www` 라 그런 것으로 보인다. 동의 메시지는 구글 CMP 의 **3선택 형식**(동의·동의하지 않음·옵션 관리)이다 — 2선택은 첫 화면에 거부 버튼이 없어서, 거부가 동의만큼 쉬워야 한다는 GDPR 원칙에 어긋난다. 미국 주 규정 메시지는 만들지 않았다(CCPA 는 매출·이용자 수 기준이 있어 대상이 아니고, 없어도 구글이 광고를 막지 않는다).
...
**화면을 통째로 다시 짰다**(2026-08-31). "딱딱하고 투박하다" 는 지적이 출발점이었고, 시안 D 의 방향은 그대로 두고 면(그림자)·활자(무게 900→700)·움직임(`lift`/`press`)·구성(홈 가운데 정렬, 목록의 `ListCard`, 정사각 출연진 카드)만 바꿨다 — 근거는 위 "면·활자·움직임" 절. 상단 바의 프로그램 링크도 이때 뺐다(프로그램이 계속 늘어서 어느 개수에서 반드시 넘친다).
...
**`lastVerified`·`source` 는 화면에 안 나온다 — 그렇게 적어 두기로 했다**(2026-09-03). `found` 384건이 전부 그 둘을 갖고 있지만 카드는 핸들만 보여 준다. `source` 가 한국어 산문 384개라 en·ja 로 옮길 표를 만들 수 없고, 카드에 들어갈 길이도 아니다. 그래서 화면을 늘리는 대신 `privacy.cast1` 이 사실을 말하게 고쳤다 — "싣습니다" 가 아니라 "따로 기록해 두고, 정정 요청이 오면 그 기록을 근거로 처리합니다" 다(ko·en·ja 세 벌 + `effectiveDate`). **두 필드를 화면에 내기로 마음이 바뀌면 이 문장부터 되돌린다.**
...
검색(`SeasonSearch`)은 **목록 바로 위 검색창**이다(홈은 프로그램 진열대 위, 프로그램 화면은 기수 목록 위). 헤더 돋보기 + `⌘K` 팔레트(shadcn `command`)로 먼저 만들었다가 반려됐다 — 목록 위 검색창이 맞다. 그래서 `command`·`dialog` 는 다시 걷어냈고 `cmdk` 의존도 지웠다.
...
**남은 것은 둘이다: 백업**(아래 "데이터는 어디에 있나")**과 애드센스 재신청 준비**(기수 소개는 배포 끝 → 직업·나이는 20~32기 끝, 1~19·33기 남음 → 재신청). 그 밖에 계속 하는 일은 계정 채우기(솔로지옥 2자리·나는 솔로 87자리)와 사진 올리기이고, 둘 다 이제 관리자 화면에서 한다 — 코드 작업이 아니다. 언어를 더할 때 절차는 위 "언어를 하나 더할 때", 프로그램을 더할 때는 `src/data/README.md` 의 "프로그램을 추가할 때".
...
**DB 에 있는 것과 코드에 남은 것이 갈린다.** 기준은 *"프로그램을 하나 더 추가할 때 이 문구도 새로 써야 하나"* 다.

| | 어디 | 왜 |
|---|---|---|
| 프로그램 이름·소개·기수 라벨(`program_strings`) | DB | 프로그램마다 새로 씀 |
| 이름 로마자·가타카나(`alias_translations`, 87) | DB | 출연자마다 늘어남 |
| 특집 이름(`special_translations`, 13) | DB | 기수마다 가끔 늘어남 |
| 기수 소개 문단(`season_descriptions`, 기수 × 언어) | DB | 기수마다 새로 씀. **없는 언어는 자리가 빠진다** — 한국어로 떨어뜨리지 않는다(`seasonDescription`) |
| 버튼·안내·상태 라벨·정책 본문(119줄) | `src/dictionaries/` | 프로그램이 늘어도 그대로 |
| 날짜·인원수·확인 개수·"방영 중" | 코드 포맷터 | 문구가 아니라 숫자 변환 |
...
- **로컬은 캐시 키가 서버를 켠 시각이다**(`local-${Date.now()}`). 고정값(`local`)이던 때는 DB 를 고쳐도 로컬 화면이 옛 응답을 보였고, dev 서버가 메모리에도 들고 있어 `.next/cache` 를 지워도 안 풀렸다(2026-09-22). **로컬 화면이 DB 와 다르면 dev 서버를 다시 켠다.**
