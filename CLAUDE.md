...
**운영 상태**(2026-09-03): **한 번 거절당하고 고쳐서 재신청한 상태다.** 2026-08-24 에 소유권 확인·검토 요청·GDPR 동의 메시지까지 끝냈고, 2026-09-02 에 "게시자 콘텐츠가 없는 화면에 Google 게재 광고" + "가치가 별로 없는 콘텐츠" 로 거절당했다. 같은 날 위 두 절대로 고쳐(광고를 페이지가 걸게 바꾸고, 소개·FAQ 산문을 세 언어로 넣었다) 재신청했고 지금은 재심사 결과를 기다린다.
...
**남은 것은 하나다: 백업**(아래 "데이터는 어디에 있나"). 그 밖에 계속 하는 일은 계정 채우기(솔로지옥 2자리·나는 솔로 87자리)와 사진 올리기이고, 둘 다 이제 관리자 화면에서 한다 — 코드 작업이 아니다. 언어를 더할 때 절차는 위 "언어를 하나 더할 때", 프로그램을 더할 때는 `src/data/README.md` 의 "프로그램을 추가할 때".
...
- **백업이 없다 — 이게 지금 남은 유일한 할 일이다.** 원본이 Supabase 하나뿐인데, 거기 든 게 데이터만이 아니라 **화면 문구 60줄과 번역 200개까지**다. `found` 384건은 계정을 직접 열어 확인해 쌓은 거라 잃으면 다시 못 만든다. `src/data/*.json` 은 **2026-09-07 이관 시점에 멈춘 사본**이고 **문구·번역은 거기에도 없다**(코드에서 지웠으니 git 이력에만 남아 있다). 붙이는 방법은 `nukko-admin-api` 레포 README 의 "아직 안 한 것 — 백업" 에 있다.
