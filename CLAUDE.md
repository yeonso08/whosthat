...
**재심사를 앞두고 본 위험 셋 중 하나는 고쳤다.** ~~① 홈에 광고가 없다~~ — 2026-09-03 에 도로 걸었다(위 "광고" 절). 남은 둘은 이렇다: ② **검증 근거가 화면에 하나도 안 나온다** — `found` 384건이 전부 `lastVerified`·`source` 를 갖고 있는데 카드는 핸들만 보여 준다. 이 사이트가 집계 블로그와 다른 유일한 점이 심사자 눈에 안 보이고, `/privacy` 의 `cast1` 이 "그 계정을 언제 어떻게 확인했는지를 싣습니다" 라고 말하는 것과도 어긋난다. ③ **이미지가 0장이다** — 출연자 사진도 프로그램 포스터도 아직 한 장도 없어서 전 화면이 활자뿐이다.
