...
- **나는 솔로 출연자 직업·나이** — 코드는 이미 있다(DB 칸·관리자 폼·카드·처리방침 문구). 408명분 데이터 조사만 남았다. 직업은 번역표가 없어 영어·일본어 화면에도 한국어로 나간다(솔로지옥이 이미 그렇다).
...
**기수 소개 — 어디까지 왔나**(2026-09-21). **코드는 세 레포 모두 머지됐다**(nukko-admin-api#1, nukko-admin#1, whosthat#59·#60). 누꼬·관리자는 Vercel 이 배포했지만 **화면은 그대로다** — DB 에 테이블도 문단도 아직 없어서 섹션이 안 그려진다. 남은 건 운영 작업 넷이고 **순서를 지켜야 한다**:

1. **Supabase SQL Editor 에서 `nukko-admin-api/schema.sql` 끝의 `season_descriptions` 를 실행한다.**
2. **OCI 에서 API 를 새 코드로 올린다**(`git pull` → `sudo systemctl restart nukko-admin-api`). 1 보다 먼저 하면 `GET /programs` 가 없는 테이블을 읽다 깨지고, 그 순간부터 누꼬 빌드·재검증이 전부 깨진다. **API 는 자동 배포가 없다** — 머지만으로는 OCI 가 안 바뀐다.
3. **초안을 넣는다** — `python -m scripts.import_season_descriptions`(목록 확인) → `--apply`. 솔로지옥 시즌 1(`review: true`)은 넣기 전에 네 번째 커플을 확인한다.
4. **누꼬를 다시 굽는다** — 스크립트는 재검증 신호를 안 보내므로 Vercel 에서 Redeploy 한 번(또는 관리자에서 기수 소개를 하나씩 저장).

그다음 기수 상세(`/ko/i-am-solo/seasons/s22`)에서 격자 아래 문단을 **눈으로 확인한다** — 로컬 API 가 없어 화면을 한 번도 못 본 채 머지했다. 그 뒤가 직업·나이 408명, 마지막이 애드센스 재신청(색인 후).
...
**남은 것은 둘이다: 백업**(아래 "데이터는 어디에 있나")**과 애드센스 재신청 준비**(위 "기수 소개 — 어디까지 왔나" 의 운영 작업 넷 → 직업·나이 채우기 → 재신청). 그 밖에 계속 하는 일은 계정 채우기(솔로지옥 2자리·나는 솔로 87자리)와 사진 올리기이고, 둘 다 이제 관리자 화면에서 한다 — 코드 작업이 아니다. 언어를 더할 때 절차는 위 "언어를 하나 더할 때", 프로그램을 더할 때는 `src/data/README.md` 의 "프로그램을 추가할 때".
