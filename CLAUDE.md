...
**프로그램은 둘이다**(2026-08-31). 나는 솔로는 1~33기 골격과 명단이 다 들어가 있고(408명, 2026-08-21) 계정은 320명이 `found`, 나머지가 `searching` 이다 — 아직 못 채운 건 방영 중이라 계정이 잠긴 33기와 각 기수에 한둘씩 남은 자리다. **솔로지옥은 2026-09-02 에 시즌 1~5 명단 66명을 채웠다**(직업까지, 계정은 전부 `searching`) — 색인에서 빠져 있던 상태가 그때 풀렸다. 명단은 나무위키와 한국 기사로 교차 확인했다. **영어 위키백과의 시즌 2·3 여성 명단은 틀렸다**(`서서은`→실제 `최서은`, `이민수`→`임민수`, `이시은`→`유시은`, `김민지`→`조민지`) — 영어 자료만 보고 채우지 말 것. 2026-09-02 에 시즌 5 부터 계정을 돌려 13/15 를 확보했다 — 전부 종영 인사 게시물이다("안녕하세요 ○○입니다… 솔로지옥5"). **실명 방송이라 나는 솔로보다 확보가 쉽다**: 가명과 달리 이름으로 바로 검색이 되고, 넷플릭스 출연자는 종영 직후 인사 글을 거의 다 올린다. 김재진·조이건은 본인이 밝힌 근거가 없어 `searching` 으로 남겼다 — 계정 후보는 있지만(조이건은 팔로워 64.9만 실명 계정) 프로필·게시물 어디에도 솔로지옥을 안 걸어 뒀다. 다음 일은 시즌 4→1 이다.
