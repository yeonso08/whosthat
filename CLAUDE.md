...
**재심사에서 남은 위험 셋**(고칠지 말지는 아직 결정 안 했다): ① **홈에 광고가 없다** — 애드센스에 등록된 사이트가 apex(`nukko.net`)라 심사 봇이 첫 화면(홈)에서 광고 코드를 못 본다. 홈은 "게시자 콘텐츠가 없는 화면" 이 아니므로(문구 한 줄 + 방영 중 줄 + 프로그램 카드) 다시 걸어도 위반이 아니다. ② **검증 근거가 화면에 하나도 안 나온다** — `found` 384건이 전부 `lastVerified`·`source` 를 갖고 있는데 카드는 핸들만 보여 준다. 이 사이트가 집계 블로그와 다른 유일한 점이 심사자 눈에 안 보이고, `/privacy` 의 `cast1` 이 "그 계정을 언제 어떻게 확인했는지를 싣습니다" 라고 말하는 것과도 어긋난다. ③ **이미지가 0장이다** — 출연자 사진도 프로그램 포스터도 아직 한 장도 없어서 전 화면이 활자뿐이다.
