...
/**
 * 애드센스 게시자 ID(`ca-pub-...`). 애드센스 콘솔에서 받아 넣는다.
 *
 * **비어 있으면 광고 스크립트를 아예 안 건다.** 틀린 ID 로 요청이 나가면
 * 애드센스가 정책 위반으로 잡으므로, 값이 없을 때는 안 붙는 쪽이 맞다.
 *
 * 값을 넣을 때 `public/ads.txt` 도 같은 pub 번호로 함께 만든다 — 둘이
 * 어긋나면 애드센스가 "승인되지 않은 판매자" 로 경고하고 수익이 막힌다.
